...
def getStopList(data,read=0):
    ...
    #     stop = Bus_stop(id=str(stops.iloc[i]['stop_id']),lat=stops.iloc[i]['stop_lat'],lon=stops.iloc[i]['stop_lon'])
    #     activate_stop = stop_times[stop_times['stop_id']==stops.iloc[i]['stop_id']]
    #     activate_trips = set(activate_stop.iloc[:]['stop_id'])
    #
    #     df1 = trips[trips['trip_id'].apply((lambda x: x in activate_trips))]
    #     routes = set(df1.iloc[:]['route_id'])
    #     stop.routes = routes
    #     stop.loc = activate_stop.iloc[0]['shape_dist_traveled']
    #
    #     stop_list[str(stops.iloc[i]['stop_id'])] = stop
    # stop_list = dict(stop_list)
...
def train_result_track(eng,ep,qloss_log,ploss_log, log, name='',seed=0,mloss_log=None):
    reward_bus_wise = []
    reward_bus_wisep1 = []
    reward_bus_wisep2 = []
    rs = []
    ...
    if ep %1 == 0:
        ...
        if mloss_log!=None:
            train_log['mloss'] = [np.mean(mloss_log)]
        ...
        res['sto'] = log['sto']
        res['sth'] = log['sth']
        print('max',np.max(wait_cost))
        print('min', np.min(wait_cost))
        print('var', np.var(wait_cost))
        print(
        ...
        try:
            if ep>1:
                # train_log.to_csv(name+str(seed)+'train_log.csv',mode='a',header =False)
...
def visualize_trajectory(engine,name=''):
    ...
    # plt.scatter([i for i in range(len(engine.speed))],engine.speed)
    # plt.show()
