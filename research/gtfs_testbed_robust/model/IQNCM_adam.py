...
import math
from model.AdamOptim import AdamOptim
...
class Agent():
    def __init__(self, state_dim, name, seed, quant_num=16, n_stops=22, buslist=None, mode='n'):
        ...
        self.actor_optim_custom = [AdamOptim() for _ in range(4)]
    ...
    def learn(self, memories, batch=16, bus_id=None):
        ...
        dw1 = torch.autograd.grad(policy_loss, self.actor_for_ml.w1, create_graph=True, retain_graph=True)[0]
        db1 = torch.autograd.grad(policy_loss, self.actor_for_ml.b1, retain_graph=True)[0]
        dw2 = torch.autograd.grad(policy_loss, self.actor_for_ml.w2, retain_graph=True)[0]
        db2 = torch.autograd.grad(policy_loss, self.actor_for_ml.b2, retain_graph=True)[0]
        dw3 = torch.autograd.grad(policy_loss, self.actor_for_ml.w3, retain_graph=True)[0]
        db3 = torch.autograd.grad(policy_loss, self.actor_for_ml.b3, retain_graph=True)[0]
        dw4 = torch.autograd.grad(policy_loss, self.actor_for_ml.w4, retain_graph=True)[0]
        db4 = torch.autograd.grad(policy_loss, self.actor_for_ml.b4, retain_graph=True)[0]
        w1, b1 = self.actor_optim_custom[0].update(self.learn_count, w=self.actor_for_ml.w1, b=self.actor_for_ml.b1, dw=dw1, db=db1)
        w2, b2 = self.actor_optim_custom[1].update(self.learn_count, w=self.actor_for_ml.w2, b=self.actor_for_ml.b2, dw=dw2, db=db2)
        w3, b3 = self.actor_optim_custom[2].update(self.learn_count, w=self.actor_for_ml.w3, b=self.actor_for_ml.b3, dw=dw3, db=db3)
        w4, b4 = self.actor_optim_custom[3].update(self.learn_count, w=self.actor_for_ml.w4, b=self.actor_for_ml.b4, dw=dw4, db=db4)
