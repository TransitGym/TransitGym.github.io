...
class Agent():
    def __init__(self, state_dim, name, seed=123, n_stops=22, buslist=None ,mode='n'):
        random.seed(seed)
        self.seed = seed
        self.name = name
        self.gamma = 0.9
    ...
    def learn(self, memories, batch=16, bus_id=None):
        ...
        b_fp_pad = batch_fp
        b_nfp_pad = batch_nfp
        # b_fp_pad = pad_sequence(batch_fp, batch_first=True)
        # b_fp_pad = pack_padded_sequence(b_fp_pad, batch_mask , batch_first=True,enforce_sorted=False)

        # b_nfp_pad = pad_sequence(batch_nfp, batch_first=True)
        # b_nfp_pad = pack_padded_sequence(b_nfp_pad, batch_mask_n , batch_first=True,enforce_sorted=False)
        ...
        def critic_learn():
            ...
            Q, A, G, reg = self.critic([b_s, b_a, b_fp_pad, b_m])
            Q_, A_, G_ ,_= self.critic_target(
                [b_ns, self.actor_target(b_ns).detach(), b_nfp_pad, b_nm])  # detach from graph, don't backpropagate

            q_target = b_r + self.gamma * (G_.detach()).view(-1,  1)*(1.- b_d)
            loss_fn = nn.MSELoss()
        ...
        def actor_learn():
            ...
            policy_loss, _, _ ,_= self.critic([b_s, batch_actor_a, batch_fp_critic_t, b_m])
            loss_fn = nn.MSELoss()
