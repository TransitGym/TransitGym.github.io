...
class Bus_stop():
    def __init__(self, id, lat, lon):
        ...
        # record which route of bus is serving at each step. 0 for no bus  20220714
        self.serving_period = []
        self.pax_actual_done = []
    ...
    def get_pax(self, bus, sim_step=0):
        ...
        if bus is not None:
            for k in range(len(self.pre_gen_pax_list)):
                if self.pre_gen_pax_list[k].arr_time <= sim_step and self.pre_gen_pax_list[k].born == 0 and \
                        self.pre_gen_pax_list[k].dest in bus.left_stop \
