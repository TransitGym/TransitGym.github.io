...
def vis_train_modelwise():
    ...
    model = ['accf', 'ddpg',
             'maddpg',
             'qmix']  # accflstm: no regularization on A; accflstmwi_: regularization on A with fp dependent weight
...
def vis_shares_effect():
    ...
    # plt.legend()
    # plt.show()
    plt.bar(np.arange(len(waits_mean)), waits_mean, yerr=waits_std, align='center', alpha=0.5, ecolor='black',
            capsize=10)
    plt.show()
...
def ml_comp():
    ...
    model_name = ['NC', 'FC', 'FC+', 'CAAC_ir_io', 'CAAC_ir_mo', 'CAAC_mr_io', 'CAAC_mr_mo',
                  'CAAC_mr_mo2']  # ,'CAAC_ir_io2','CAAC_ir_mo2','CAAC_mr_io2','CAAC_mr_mo2']
    ...
    def vis_training_performance(r, item, w=2, sc=0):
        ...
        for m in range(len(models)):
            ...
            try:
                ...
                plt.plot(mean, label=model_name[model_index], linewidth=2, color=COLORS[color_index],
        ...
        def stop_wise_draw(data, name, i=0):
            ...
            for k in range(len(data)):
                ...
                if name != 'AHT' or (name == 'AHT' and k != 0):
                    try:
                        plt.errorbar(np.arange(np.array(data[k]).shape[1])[b1:b2 + 1],
                                     np.mean(np.array(data[k]), axis=0)[b1:b2 + 1],
