...
class Actor(nn.Module):
    ...
    def forward(self, s):
        try:
            s, stop_embedding = s
            # do not use weight as input for actor in control 7
            # if int(s.size(1))==11:
            #     s = s[:,:-3]
        except:
            pass
...
class Agent():
    ...
    def choose_action(self, state ):
        if self.is_stop_embedding==1:
            s,stop_embed =state
            state = [torch.tensor(s, dtype=torch.float).unsqueeze(0),torch.tensor(stop_embed, dtype=torch.float).unsqueeze(0)]
        else:
    ...
    def learn(self, memories, batch=16, bus_id=None):
        ...
        for s, fp, a, r, ns, nfp,embed,next_embed in memory:
            ...
            if self.is_stop_embedding==1:
                fp_[0, self.state_dim + 1] = self.actor([torch.tensor(s, dtype=torch.float32),torch.tensor(embed, dtype=torch.float32)]).detach()
            else:
                fp_[0, self.state_dim+1] = self.actor( torch.tensor(s, dtype=torch.float32) ).detach()
            batch_fp_critic_t.append(fp_)
            batch_actor_a.append(self.actor([torch.tensor(s, dtype=torch.float32),torch.tensor(embed, dtype=torch.float32)]))
