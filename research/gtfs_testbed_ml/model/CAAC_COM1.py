...
class Actor(nn.Module):
    def __init__(self, input_size, hidden_size=400, output_size=1,is_stop_embedding=False ):
        ...
        self.linear2 = nn.Linear(hidden_size+32, hidden_size)
    ...
    def forward(self, s):
        x,com = s
        com = com.reshape(-1,5)
        embed_com = self.relu(self.embed(com))
        embed_com = embed_com.reshape(x.size(0),-1,32)

        pool1d = torch.nn.MaxPool1d(kernel_size=embed_com.size(1), stride=1, padding=0, )
        embed_com = pool1d(embed_com.permute(0, 2, 1)).permute(0, 2, 1)
        embed_com = embed_com.reshape(-1,32)

        x = self.elu(self.linear1(x))

        x = torch.cat([x,embed_com],1)
...
class Critic(nn.Module):
    def __init__(self, state_dim, n_stops=22, action_dim=1, is_stop_embedding=False):
        ...
        self.u_attentions = [layers.GraphAttentionLayer(state_dim+1+2, self.hidden1, dropout=False, alpha=0.2, concat=True) for _ in
                           range(1)]
        for i, attention in enumerate(self.u_attentions):
            self.add_module('attention_{}'.format(i), attention)
        self.u_out_att = layers.GraphAttentionLayer(self.hidden1 * 1, self.hidden1, dropout=False, alpha=0.2, concat=False)

        self.d_attentions = [layers.GraphAttentionLayer(state_dim+1+2, self.hidden1, dropout=False, alpha=0.2, concat=True) for _ in
                           range(1)]
        for i, attention in enumerate(self.d_attentions):
            self.add_module('attention_{}'.format(i), attention)
        self.d_out_att = layers.GraphAttentionLayer(self.hidden1 * 1, self.hidden1, dropout=False, alpha=0.2, concat=False)
    ...
    def forward(self, xs):
        x, a, fp, embed = xs
        ego = torch.cat([x, a.reshape(-1,1)], 1)
...
class Agent():
    ...
    def choose_action(self, state ):
        s,com =state
        state = [torch.tensor(s, dtype=torch.float).unsqueeze(0),torch.tensor(com, dtype=torch.float).unsqueeze(0)]
    ...
    def learn(self, memories, batch=16, bus_id=None):
        ...
        for s, fp, a, r, ns, nfp,embed,next_embed in memory:
            s,com = s
            com_ = [0. for _ in range(100)]
            com_[:len(com)] = com
            batch_s.append(s)
            nscom_ = [0. for _ in range(100)]
            ns,nscom = ns
            nscom_[:len(nscom)] = nscom
            fp_ = copy.deepcopy(fp)
            fp_ = torch.tensor(fp_, dtype=torch.float32)
            a_t = self.actor([torch.tensor([s], dtype=torch.float32),torch.tensor(com, dtype=torch.float32)])
            fp_[0, self.state_dim+1] = a_t.detach()
            batch_fp_critic_t.append(fp_)
            batch_actor_a.append(a_t)
            ...
            batch_r.append(r)
            batch_ns.append(ns)
            batch_embed.append(com_)
            batch_next_embed.append(nscom_)
        ...
        def critic_learn():
            ...
            Q_, A_, G_ ,_= self.critic_target(
                [b_ns, self.actor_target(b_ns_comb).detach(), b_nfp_pad, b_nd])  # detach from graph, don't backpropagate
